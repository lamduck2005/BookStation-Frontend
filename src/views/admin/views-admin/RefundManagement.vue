<template>
  <div class="container-fluid py-4">
    <!-- Breadcrumb -->
    <div class="mb-3">
      <h6 class="text-muted">Admin / <strong>Quản lý hoàn hàng</strong></h6>
    </div>
            ...
            <div>
              <div class="table-responsive">
                <table class="table align-middle table-hover mb-0">
                  ...
                  <tbody>
                    <tr
                      v-for="(refund, index) in filteredRefunds"
                      :key="refund.id"
                    >
                      <td>
                        {{
                    ...
                    <tr v-if="filteredRefunds.length === 0">
                      <td colspan="9" class="text-center text-muted py-4">
                        <i
                          class="bi bi-inbox display-1 text-muted d-block mb-3"
                        ></i>
              ...
              <table class="table table-sm">
                <tbody>
                  ...
                  <tr>
                    ...
                    <td>
                      <span class="badge bg-info">{{
                        selectedRefund.trackingCode || "N/A"
                      }}</span>
                    </td>
                  </tr>
                  <tr>
                  ...
                  <tr>
                    ...
                    <td>
                      <span
                        class="badge"
                        :class="
                          selectedRefund.refundType === 'FULL'
              ...
              <table class="table table-sm">
                <tbody>
                  <tr>
                    ...
                    <td>
                      {{
                        selectedRefund.userFullName ||
                        selectedRefund.orderDetails?.userFullName
                      }}
                    </td>
                  </tr>
                  <tr>
                  ...
                  <tr v-if="selectedRefund.orderDetails?.deliveredDate">
                    ...
                    <td>
                      {{
                        formatDate(selectedRefund.orderDetails.deliveredDate)
                      }}
                    </td>
                  </tr>
                  ...
                  <tr v-if="selectedRefund.remainingDays !== undefined">
                    ...
                    <td>
                      <span
                        :class="
                          selectedRefund.remainingDays > 3
                            ? 'text-success'
                            : 'text-danger'
                        "
                      >
          ...
          <div class="row">
            <div class="col-12">
                ...
                <tbody>
                  <tr>
                    ...
                    <td>
                      ...
                      {{ formatTime(selectedRefund.createdAt) }}
                    </td>
                  </tr>
                  <tr>
                    ...
                    <td>
                      <StatusLabel
                        :status="
                          selectedRefund.refundStatus || selectedRefund.status
                        "
                        :type="
                          getStatusType(
                            selectedRefund.refundStatus || selectedRefund.status
                      ...
                      <span class="ms-2 text-muted">{{
                        selectedRefund.refundStatusDisplay ||
                        selectedRefund.statusDisplay
                      }}</span>
                    </td>
                  </tr>
                  <tr>
              ...
              <div v-if="selectedRefund.customerNote" class="mt-3">
                ...
                <div
                  class="bg-info bg-opacity-10 p-3 rounded border-start border-info border-4"
                >
              ...
              <div v-if="selectedRefund.adminNote" class="mt-3">
                ...
                <div
                  class="bg-warning bg-opacity-10 p-3 rounded border-start border-warning border-4"
                >
                  {{ selectedRefund.adminNote }}
                </div>
                <small class="text-muted">
                  Bởi: {{ selectedRefund.approvedByName || "Admin" }}
                  <span v-if="selectedRefund.approvedAt">
                    - {{ formatDate(selectedRefund.approvedAt) }}</span
                  >
                </small>
              </div>
              ...
              >
                ...
                <div class="table-responsive">
                  <table class="table table-sm table-bordered">
                    ...
                    <tbody>
                      <tr
                        v-for="item in selectedRefund.refundItems"
                        :key="item.bookId"
                      >
                        <td>
              ...
              >
                ...
                <div class="timeline">
                  <div
                    v-for="(event, index) in selectedRefund.timeline"
                    :key="index"
                    class="timeline-item"
                  >
                    <div
                      class="timeline-marker"
                      :class="getTimelineMarkerClass(event.status)"
                    ></div>
                    <div class="timeline-content">
                      ...
                      <div v-if="event.adminName" class="text-muted small">
                        Bởi: {{ event.adminName }}
                      </div>
                    </div>
                  </div>
              ...
              <div v-if="selectedRefund.adminInfo" class="mt-4">
                ...
                <div class="alert alert-light">
                  <p>
                    <strong>Có thể phê duyệt:</strong>
                    <span
                      :class="
                        selectedRefund.adminInfo.canApprove
                          ? 'text-success'
                          : 'text-danger'
                      "
                    >
                  ...
                  <div
                    v-if="selectedRefund.adminInfo.managerApprovalRequired"
          ...
          <button
            ...
            @click="
              processRefundRequest(
                selectedRefund?.refundRequestId || selectedRefund?.id
              )
            "
            type="button"
      ...
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="evidenceModalLabel">
            Minh chứng hoàn hàng -
            {{ evidenceType === "images" ? "Hình ảnh" : "Video" }}
          </h5>
          <button
            type="button"
            class="btn-close"
        ...
        <div class="modal-body">
          <div v-if="evidenceType === 'images'" class="row">
            <div
              v-for="(image, index) in currentEvidence"
              :key="index"
              class="col-md-4 mb-3"
            >
              <img
          ...
          <div v-else-if="evidenceType === 'videos'" class="row">
            <div
              v-for="(video, index) in currentEvidence"
              :key="index"
              class="col-md-6 mb-3"
            >
              <video
...
const filteredRefunds = computed(() => {
  ...
  if (filters.refundType) {
    result = result.filter(
      (refund) => refund.refundType === filters.refundType
    );
  }

  if (filters.startDate) {
    result = result.filter(
      (refund) => new Date(refund.createdAt) >= new Date(filters.startDate)
    );
  }

  if (filters.endDate) {
    result = result.filter(
      (refund) => new Date(refund.createdAt) <= new Date(filters.endDate)
    );
  }
  ...
  if (filters.minAmount) {
    result = result.filter(
      (refund) => refund.totalRefundAmount >= parseInt(filters.minAmount)
    );
  }

  if (filters.maxAmount) {
    result = result.filter(
      (refund) => refund.totalRefundAmount <= parseInt(filters.maxAmount)
    );
  }
...
const fetchRefundRequests = async () => {
  ...
  try {
    ...
    if (filters.status && filters.status !== "" && filters.status !== "ALL") {
      ...
      console.log("Response with status filter:", response);
    } else {
...
const viewEvidence = (refund, type) => {
  ...
  if (type === "images") {
    currentEvidence.value =
      refund.evidenceFiles?.images || refund.evidenceImages || [];
  } else {
    currentEvidence.value =
      refund.evidenceFiles?.videos || refund.evidenceVideos || [];
  }
...
const approveRefundRequest = async (refund) => {
        ...
        <div>
          ${refund.refundItems
            .map(
              (item, idx) => `
            <div class="mb-2">
              <span><strong>${item.bookTitle}</strong> (ID: ${
                item.bookId
              })</span><br>
              <input id="refundQuantity_${idx}" type="number" class="form-control mt-1" value="${
                item.refundQuantity || item.quantity || 1
              }" min="1" max="${
                item.quantity || 10
  ...
  const { value: formValues } = await Swal.fire({
    ...
    text: `Phê duyệt hoàn hàng ${formatCurrency(
      refund.totalRefundAmount
    )} cho đơn hàng #${refund.orderCode}`,
    html: `
      <div class="mb-3">
    ...
    preConfirm: () => {
      ...
      if (approvedAmount > refund.totalRefundAmount) {
        Swal.showValidationMessage(
          "Số tiền hoàn không được vượt quá số tiền yêu cầu"
        );
        return false;
      }
      ...
      if (refund.refundItems && refund.refundItems.length > 0) {
        refundItems = refund.refundItems.map((item, idx) => {
          const refundQuantity =
            parseInt(document.getElementById(`refundQuantity_${idx}`).value) ||
            1;
          return {
            ...item,
            refundQuantity,
  ...
  if (formValues) {
    try {
      ...
      const errorMessage =
        ...
        err.message ||
        "Có lỗi xảy ra khi phê duyệt hoàn hàng";
      Swal.fire({
        title: "Lỗi!",
        text: errorMessage,
        icon: "error",
  ...
  if (formValues) {
    try {
      ...
      const errorMessage =
        err.response?.data?.message ||
        err.message ||
        "Có lỗi xảy ra khi từ chối hoàn hàng";
      Swal.fire({
        title: "Lỗi!",
        text: errorMessage,
        icon: "error",
...
const processRefundRequest = async (refundId) => {
  ...
  const { value: formValues } = await Swal.fire({
    ...
    preConfirm: () => {
      ...
      const refundFee =
        parseInt(document.getElementById("refundFee").value) || 0;
  ...
  if (formValues) {
    try {
      ...
      const errorMessage =
        err.response?.data?.message ||
        err.message ||
        "Có lỗi xảy ra khi xử lý hoàn trả";
      Swal.fire({
        title: "Lỗi!",
        text: errorMessage,
        icon: "error",
