  ...
  <div class="product-card h-100">
    ...
    >
      ...
      <div class="position-relative">
        <!-- Flash Sale badge -->
        <span
          v-if="product.isInFlashSale"
          class="badge bg-danger position-absolute top-0 start-0 m-2 z-index-1 sale-badge flash-sale-badge"
        >
        ...
        <span
          ...
          "
          class="badge bg-warning position-absolute top-0 start-0 m-2 z-index-1 sale-badge text-dark"
        >
          {{ getDisplayLabel() }}
        </span>
      ...
      <div class="card-body p-2 d-flex flex-column">
        ...
        <div class="price-section mt-auto mb-2">
          <div class="d-flex align-items-center gap-2">
            <!-- Flash Sale Price -->
            <span
              v-if="product.isInFlashSale && product.flashSalePrice"
              class="current-price fw-bold text-danger fs-6"
            >
            ...
            <small class="text-muted" style="font-size: 0.8rem">
              ...
              <span
                v-if="
                  ...
                  product.flashSaleSoldCount !== undefined
                "
              >
              ...
              <span v-else>
                Đã bán
                {{
                  product.soldCount || product.soldQuantity || product.sold || 0
                }}
              </span>
            </small>
          </div>
...
const getOriginalPrice = () => {
  ...
  if (props.product.isInFlashSale && props.product.flashSalePrice) {
    // Nếu có trường originalPrice thì dùng, nếu không thì dùng price
    return props.product.originalPrice ?? props.product.price ?? false;
  }
  // Nếu không phải flash sale, nếu có originalPrice thì dùng, nếu không thì dùng price
  return false;
};
...
const getSalesProgress = () => {
  ...
  if (props.product.isInFlashSale) {
    ...
    total = getFlashSaleStockQuantity();
  } else {
    sold =
...
// Thay mọi chỗ dùng product.flashSaleStockQuantity bằng hàm này:
const getFlashSaleStockQuantity = () => {
  return (
    props.product.flashSaleStockQuantity ?? props.product.flashSaleStock ?? 0
  );
};
</script>
